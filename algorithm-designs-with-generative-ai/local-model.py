...
# Display available algorithms to the user
...
while True:
    ...
    selection = int(input("Choose an algorithm (enter corresponding number): ")) - 1
    selected_algorithm = algorithms[selection]
    # Pass the selected algorithm to the Python program
    alog_name = selected_algorithm["name"]
    alog_var = selected_algorithm["algorithm"]
    var = "Please create an algorithm for " + alog_name + " using " + alog_var + " technique"

    # Ask the user if they want to generate test cases
    test_selection = input("Would you like to generate test cases as well? (yes/no): ")

    if test_selection.lower() == "yes":
        # Include test cases
        var = var + ".Please include test cases also"
    elif test_selection.lower() == "no":
        var = var
    else:
        print("Invalid input. Please enter 'yes' or 'no'.")

    print("Selected Algorithm: " + var)
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "Always answer in python."},
